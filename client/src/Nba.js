...
class Nba extends Component {
  ...
  handleIDsSubmit(event) {
    ...
    const fetchGame = (gameID) => {
      var url = '/pregame?league=nba&id=' + gameID;
      axios.get(url)
  ...
  render() {
    return (
      <div className="nba">
      <h1 style={{"textAlign": "left"}}>NBA</h1>
        <div>
          {this.state.games.map(game =>
            <Game game={game} key={game.id} league="nba" clicked={this.state.clicked} delay={this.state.updateMultipleDelay} />
          )}
        </div>
